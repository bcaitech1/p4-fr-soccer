...
def collate_batch(data):    #transformer 등에 사용(데이터 형태가 일반 모델과 다름 + 자연어처리에서 padding을 배치당 맥스 랭스로 줄 때
...
class LoadDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if self.transform:
            image = self.transform(image=image)['image'][:1]
        image = image.float()/255
        return {"path": item["path"], "truth": item["truth"], "image": image.float()}
...
class LoadEvalDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        return {"path": item["path"], "file_path":item["file_path"],"truth": item["truth"], "image": image.float()}

def dataset_loader(options, train_transformed, test_transformed):
    ...
    valid_dataset = LoadDataset(
        valid_data, options.data.token_paths, crop=options.data.crop, transform=test_transformed, rgb=options.data.rgb
    )
