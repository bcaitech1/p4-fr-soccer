...
from dataset_ViT import dataset_loader, START, PAD, load_vocab
...
from dataset_ViT import START, PAD, END
...
def run_epoch(
    ...
    with tqdm(
        ...
        for d in data_loader:
            ...
            loss = decoder(expected, mask=None, context=encoded)
            sequence = decoder.generate(torch.LongTensor([model.bos_token] * len(input))[:, None].to(device), length-1,
                                    eos_token=model.eos_token, context=encoded, temperature=model.temperature)

            diff = expected.shape[1] - sequence.shape[1] - 1
            if diff >= 1:
                pad_tensor = torch.tensor(
                    [[data_loader.dataset.token_to_id[PAD] for _ in range(diff)] for _ in range(sequence.shape[0])]).to(
                    device)
                sequence = torch.cat((sequence, pad_tensor), 1)
...
def main(config_file):
    ...
    for epoch in range(options.num_epochs):
        ...
        epoch_score = 0.9 * validation_epoch_sentence_accuracy + 0.1 * (1 - validation_epoch_wer)
